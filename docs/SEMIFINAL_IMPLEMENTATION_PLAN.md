...
- End-to-end policy-to-receipt integration test:
  PaymentRequest -> two approvals -> PaymentIntent -> exact ALLOW -> modified amount DENY -> quote -> prepare/sign -> no-broadcast receipt.
...
- There is no separate WDK "prepare transaction" API in the installed EVM wallet. CupTreasury will prepare a deterministic ERC-20 transaction object and use WDK quote/sign APIs around it.
