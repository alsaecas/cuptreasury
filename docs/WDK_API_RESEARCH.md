...
| Fee quote for ERC-20 transfer | `@tetherto/wdk-wallet-evm` | `account.quoteTransfer({ token, recipient, amount })`; `account.quoteSendTransaction(tx)` for prepared ERC-20 calldata | Partial | Attempt to quote the exact prepared transaction after policy simulation. | Quote is read-only and not WDK policy-wrapped. The current placeholder token has no bytecode and an unfunded ephemeral account, so the proof records unsupported quote status instead of faking success. |
